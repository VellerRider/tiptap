  ...
  return (SlashMenu as unknown as Extension).configure({
    ...
    items: ({ query, editor: ed }: Parameters<NonNullable<SuggestionOptions['items']>>[0]) => {
      const all = [
        ...
        // The following commands are provided by optional extensions (BulletList, CodeBlock, HorizontalRule).
        // Cast to any so the package types don't force consumers to include those extension typings.
        { id: 'bullet', title: 'Bullet List', run: () => (ed as any).chain().focus().toggleBulletList().run() },
        { id: 'code', title: 'Code Block', run: () => (ed as any).chain().focus().toggleCodeBlock().run() },
        { id: 'hr', title: 'Horizontal Rule', run: () => (ed as any).chain().focus().setHorizontalRule().run() },
      ]
